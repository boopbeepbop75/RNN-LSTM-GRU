...
#Initialize model
model = Model.SimpleLSTM(input_dim=9)
...
for epoch in range(H.EPOCHS):
    ...
    if testing_loss < best_val_loss:
        ...
        torch.save(model.state_dict(), (U.MODEL_FOLDER / (H.LSTM_MODEL_NAME + '.pth')).resolve())
        with open((U.MODEL_FOLDER / (H.LSTM_MODEL_NAME + '_loss.txt')).resolve(), 'w') as f:
